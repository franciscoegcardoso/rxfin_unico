...
export interface RecurringCardWithCardProps {
...
/** Variant for cartão: labels "✓ Na fatura" / "Aguardando fatura" and optional card_name badge. */
export const RecurringCardWithCard: React.FC<RecurringCardWithCardProps> = ({
  item,
  cardName,
  className,
}) => {
  ...
  const seenLabel = '✓ Na fatura';
  const pendingLabel = 'Aguardando fatura';
  const badgeVariant = item.seen_this_month ? 'success' : 'warning';
