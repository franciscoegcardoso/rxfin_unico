...
export const ControlOnboardingBanner: React.FC = () => {
  ...
  const [dismissed, setDismissed] = useState(() =>
    typeof sessionStorage !== 'undefined' && sessionStorage.getItem('control-banner-dismissed') === 'true'
  );

  if (dismissed) return null;

  const handleDismiss = () => {
    if (typeof sessionStorage !== 'undefined') sessionStorage.setItem('control-banner-dismissed', 'true');
    setDismissed(true);
  };
