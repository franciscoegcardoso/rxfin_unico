...
export const BudgetInsightsSection: React.FC = () => {
  ...
  const financialData = useMemo(() => {
    // Helper to ensure we always have a number (default to 0)
    const safeNumber = (val: number | undefined | null): number => val ?? 0;

    // Recurring expenses from expense items
    ...
    // Get installments from contas (parceladas)
    ...
    // Credit card total for current month
    ...
    // Monthly budget (total projected expenses)
  ...
  // Fetch insights on mount if we have data
  useEffect(() => {
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-lg">Insights do Mês</CardTitle>
          </div>
          <div className="flex items-center gap-2">
      ...
      <CardContent className="space-y-4">
        {loading ? (
          ...
          <>
            ...
            >
              <span className="text-sm">Ver análise completa</span>
