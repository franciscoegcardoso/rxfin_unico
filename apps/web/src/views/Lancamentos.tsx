  ...
  return (
    <>
      ...
      <div className="space-y-6">
        ...
        {/* RPC summary (get_lancamentos_summary) — quando disponível */}
        {rpcSummary?.summary != null && (
          <Card className="rounded-[14px] border border-border/80 p-4">
            <div className="flex flex-wrap items-center gap-4 text-sm">
              <span className="font-medium text-muted-foreground">Resumo do mês (RPC):</span>
              <span className="text-income font-semibold">
                Receitas: {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(rpcSummary.summary.total_income)}
              </span>
              <span className="text-expense font-semibold">
                Despesas: {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(rpcSummary.summary.total_expense)}
              </span>
              <span className={cn('font-semibold', rpcSummary.summary.balance >= 0 ? 'text-income' : 'text-expense')}>
                Saldo: {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(rpcSummary.summary.balance)}
              </span>
              {rpcSummary.summary.count != null && (
                <span className="text-muted-foreground">{rpcSummary.summary.count} lançamento(s)</span>
              )}
            </div>
            {rpcSummaryError && (
              <p className="text-xs text-destructive mt-2">Erro ao carregar resumo: {rpcSummaryError}</p>
            )}
          </Card>
        )}
