-- Expõe metadata nas notificações para deep-link de reconexão Pluggy (item_id, connector_name).
...
COMMENT ON FUNCTION public.get_notifications_page(integer, integer) IS
  'Paginated notifications for current user (including broadcasts); includes metadata for connection deep-links.';
