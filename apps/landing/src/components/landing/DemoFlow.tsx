    ...
    <section className="bg-[#0d2b20]">
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-20">
              ...
              <div className="flex flex-col items-center">
                <button
                  ...
                  className={cn(
                    ...
                    i === step ? 'bg-primary text-white scale-110 shadow-lg shadow-primary/30' : i < step ? 'bg-primary/40 text-white/70' : 'bg-white/8 text-white/25 hover:bg-white/12'
                  )}
                >
                  {i + 1}
                </button>
                <span className="text-[10px] text-white/35 mt-1 text-center w-16 hidden sm:block">{s.label}</span>
              </div>
              {i < STEPS.length - 1 && (
                <div className={cn('flex-1 h-px mx-1 min-w-[12px]', i < step ? 'bg-primary/40' : 'bg-white/8')} />
              )}
            </React.Fragment>
          ))}
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center min-h-[420px]">
          <div>
            <p className="text-white/40 text-xs font-medium mb-2">{content.label}</p>
            <h3 className="text-xl lg:text-2xl font-bold text-white mb-3">{content.title}</h3>
            <p className="text-white/70 text-sm mb-4">{content.desc}</p>
        ...
        <div className="flex items-center justify-center gap-6 mt-12">
          <button
            ...
            disabled={step === 0}
            className="p-2 rounded-lg bg-white/10 text-white disabled:opacity-40 disabled:cursor-not-allowed hover:bg-white/15 transition-colors"
          >
            <ChevronLeft className="w-5 h-5" />
          </button>
          <span className="text-white/30 text-sm tabular-nums">{step + 1} / 5</span>
          {step === 4 ? (
            <a
              href="https://app.rxfin.com.br"
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-primary text-white font-semibold hover:bg-primary/90 transition-colors"
            >
              Começar grátis →
            </a>
          ) : (
            <button
              ...
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-white/10 text-white hover:bg-white/15 transition-colors"
            >
              Próximo
...
function Step0Visual({
  ...
  if (authorized) {
    return (
      <div className="bg-primary/10 border border-primary/30 rounded-xl p-5 text-center w-full max-w-xs">
        <p className="font-bold text-white text-lg mb-1">✅ Conectado!</p>
        <p className="text-white/60 text-sm mb-3">Importando transações...</p>
        ...
        <div className="bg-white rounded-2xl border-4 border-gray-800 shadow-2xl overflow-hidden">
          ...
          <div className="p-4 flex flex-col gap-3">
            ...
            <button
              ...
              className="bg-primary text-white text-xs rounded-lg py-2 w-full font-medium"
            >
              Autorizar
            </button>
          </div>
  ...
  return (
    <div>
      <div className="grid grid-cols-2 gap-2 max-w-[240px] mx-auto">
        {BANKS.map((b, i) => (
          <button
            ...
            className="bg-white/5 border border-white/10 rounded-xl p-4 text-center cursor-pointer hover:border-primary/50 hover:bg-primary/10 transition-all"
          >
      ...
      <p className="text-white/25 text-xs text-center mt-3">← Clique para simular conexão</p>
    </div>
  );
}
  ...
  return (
    <div className="w-full max-w-sm space-y-1.5">
      <div className="flex items-center gap-2 mb-3">
        <span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" />
        <span className="text-white/60 text-xs">Cibélia analisando 87 transações</span>
      </div>
      {TRANSACTIONS.map((t, i) => (
        <div
          ...
          className={cn(
            ...
            revealed.includes(i) ? 'border-primary/20 bg-primary/5 border' : 'bg-white/5 border border-white/10'
          )}
        >
          <div>
            <span className="text-white/90">{t.desc}</span>
          ...
          {revealed.includes(i) ? (
            ...
            <span className="bg-white/10 text-white/20 text-[10px] rounded-full px-2 py-0.5 animate-pulse">analisando...</span>
          )}
        </div>
      ))}
      {revealed.length === TRANSACTIONS.length && (
        <button type="button" className="w-full mt-3 bg-primary text-white rounded-lg px-4 py-2 text-sm font-medium">
          ✓ Confirmar todas →
        </button>
      )}
    </div>
    ...
    <div className="max-w-sm mx-auto rounded-xl overflow-hidden shadow-2xl border border-white/10 -rotate-1 text-left">
      ...
      <div className="bg-white px-4 py-2.5 flex justify-between items-center border-b border-gray-100">
        <div className="flex items-center gap-2">
          <div className="rounded-full w-6 h-6 bg-primary text-white text-[9px] flex items-center justify-center font-bold">F</div>
  ...
  return (
    <div className="max-w-sm mx-auto flex flex-col gap-3">
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl p-4">
        <p className="text-[9px] font-bold text-white/35 uppercase mb-3">PROGRESSO DEDUÇÃO</p>
        {bars.map((b, i) => (
          <div key={i} className="mb-2">
            <div className="flex justify-between items-center mb-0.5">
              <span className="text-[10px] text-white/70">{b.icon} {b.label}</span>
              <span className={cn('text-[9px]', b.alerta ? 'text-amber-400' : 'text-white/50')}>{b.valor}</span>
            </div>
            <p className="text-[8px] text-white/40 mb-1">{b.sub}</p>
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl p-3">
        <div className="flex items-center gap-2">
          <span className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse" />
          <span className="text-[9px] text-white/50">Cibélia — Fiscal Organizer</span>
        </div>
        <div className="mt-2 flex gap-2">
          <div className="w-6 h-6 rounded-full bg-primary flex-shrink-0 flex items-center justify-center text-[9px] text-white font-bold">C</div>
          <div className="bg-primary/10 border border-primary/20 rounded-[2px_8px_8px_8px] px-3 py-2 text-[11px] text-white/75 leading-relaxed">
            Você foi ao dentista → guarde o comprovante. Despesas odontológicas são <strong className="text-white">dedutíveis sem limite</strong>. 🦷
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="max-w-sm mx-auto flex flex-col gap-3">
      <div className="bg-white/5 border border-white/10 rounded-xl p-4">
        <div className="flex justify-between items-center mb-3">
          <span className="text-[9px] font-bold text-white/35 uppercase">PROJEÇÃO DE PATRIMÔNIO</span>
          <div className="flex gap-3">
            <span className="flex items-center gap-1 text-[8px] text-white/40"><span className="w-2 h-2 rounded-full bg-primary/40 inline-block" />Conservador</span>
            <span className="flex items-center gap-1 text-[8px] text-white/40"><span className="w-2 h-2 rounded-full bg-primary inline-block" />Realista</span>
            <span className="flex items-center gap-1 text-[8px] text-white/40"><span className="w-2 h-2 rounded-full bg-green-400 inline-block" />Otimista</span>
          </div>
        </div>
        <svg viewBox="0 0 400 110" className="w-full">
          ...
          <line x1="40" y1="15" x2="390" y2="15" stroke="rgba(255,255,255,0.06)" strokeWidth="1" />
          <line x1="40" y1="50" x2="390" y2="50" stroke="rgba(255,255,255,0.06)" strokeWidth="1" />
          <line x1="40" y1="85" x2="390" y2="85" stroke="rgba(255,255,255,0.06)" strokeWidth="1" />
          <text x="2" y="18" fontSize="7" fill="rgba(255,255,255,0.25)">R$5M</text>
          <text x="2" y="53" fontSize="7" fill="rgba(255,255,255,0.25)">R$3M</text>
          <text x="2" y="88" fontSize="7" fill="rgba(255,255,255,0.25)">R$1M</text>
          <text x="38" y="105" fontSize="7" fill="rgba(255,255,255,0.25)">Hoje</text>
          <text x="148" y="105" fontSize="7" fill="rgba(255,255,255,0.25)">2027</text>
          <text x="248" y="105" fontSize="7" fill="rgba(255,255,255,0.25)">2036</text>
          <text x="360" y="105" fontSize="7" fill="rgba(255,255,255,0.25)">2056</text>
        ...
        {[
          ...
          <div
            ...
            className={cn(
              ...
              destaque ? 'bg-primary/10 border-primary/40' : 'bg-white/5 border-white/10'
            )}
          >
            ...
            {destaque && <p className="text-[7px] text-white/25">cenário realista</p>}
          </div>
        ))}
      </div>
    </div>
